...
class Template{
    ...
    public function theme($pages, $data = NULL)
    {
        $folder = 'backend/';

        $tpl['content']     = $this->ci->load->view($folder . 'contents/' . $pages, $data, true);
        $tpl['navbar']      = $this->ci->load->view($folder . 'header', null, true);

        $this->ci->load->view($folder . 'template', $tpl);
    }

    public function umum($pages, $data = NULL)
    {
        $folder = 'backend/';

        $tpl['content']     = $this->ci->load->view($folder . 'contents/' . $pages, $data, true);
        $tpl['navbar']      = $this->ci->load->view($folder . 'header_umum', null, true);
