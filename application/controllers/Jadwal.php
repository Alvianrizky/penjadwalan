...
class Jadwal extends CI_Controller {
	...
	protected $page_header = 'Jadwal Management';
	...
	{  
		...
		$this->template->theme('jadwal_v', $data);
	}
								...
								if (is_array($pisah) == 1) {
									...
									if (!empty($ruang_in[0][$l]['nilai'])) {
										$in_data = [
											...
											'onClick' => 'update_data(' . $ruang_in[0][$l]['idPengampu'] . ')'
											...
											if (!empty($ruang_in[$k][$m]['nilai'])) {
												$in_data = [
													'data' => $makul->namaMakul .
														...
														'<br>' . $dosen->namaDosen,
													'class' => 'bg bg-danger text-white',
													'onClick' => 'update_data(' . $ruang_in[$k][$m]['idPengampu'] . ')'
