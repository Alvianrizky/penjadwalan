...
class Tes extends CI_Controller {
	...
	{
		$pengampu = $this->gabung();

		$hasil = [];
		for($i = 0; $i < count($pengampu); $i++) {
			$makul = $this->makul->where('id', $pengampu[$i]['idMakul'])->get();
			$dosen = $this->dosen->where('id', $pengampu[$i]['idDosen'])->get();

			if(is_array($pengampu[$i]['idKelas'])) {
				$kelas1 = $this->kelas->where('id', $pengampu[$i]['idKelas'][0])->get();
				$kelas2 = $this->kelas->where('id', $pengampu[$i]['idKelas'][1])->get();

				$hasil[$i] = [
					'id' => $pengampu[$i]['id'],
					'Dosen' => $dosen->namaDosen,
					'Makul' => $makul->namaMakul,
					'Kelas' => [
						0 => $kelas1->namaKelas,
						1 => $kelas2->namaKelas
					]
				...
				$kelas = $this->kelas->where('id', $pengampu[$i]['idKelas'])->get();
